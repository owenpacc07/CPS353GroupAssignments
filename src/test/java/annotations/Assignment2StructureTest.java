 ...
 * have corresponding prototypes,
 * also with the appropriate annotations.
 * 
 * Remember, it's easiest to build the prototype first, and use that to create the APIs
 * - use Eclipse to auto-generate classes, 
 * interfaces, etc as you build out the prototype. This ensures that
 * your API is written from the perspective of the client - the prototype
...
public class Assignment2StructureTest {
	...
	public void checkPrototypesExist(Class<? extends Annotation> apiAnnotation,
					...
					if (m.isAnnotationPresent(prototypeAnnotation)) {
						...
						if (parameters.length != 1) {
							errors.add("Prototype method should have a single parameter, the API being prototyped. "
									+ "If you need other data to simulate classes/objects"
									+ "that the client will receive from elsewhere"
									+ " (a website form, another componenet, etc), just create those"
									+ "as local variables set to null/0 for now. "
		...
		if (numPrototypesFound != 1) {
			errors.add("No (or multiple) class method with the " +
					prototypeAnnotation.getSimpleName() + " annotation was found in 'src'");
		}
		if (!errors.isEmpty()) {
	...
	public void checkAnnotationsExist(Class<? extends Annotation> apiAnnotation)
		...
		if (numApisFound != 1) {
			errors.add("No (or multiple) interface found with annotation "
		+ apiAnnotation.getSimpleName() + " in 'src'");
			errors.add("Keep in mind that all APIs must be interfaces"
					+ " (not classes), and that each API annotation"

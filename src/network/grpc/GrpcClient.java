...
public class GrpcClient {
			...
			if (userresponse.equals("0")) {
				...
				if (!(userresponse.equals("C") || fileInput)) {
					...
					throw new IllegalArgumentException(
							"Picked an unavailable option, choices were file input (F) or console input (C)");
				}
				...
				channel.shutdown();
				throw new IllegalArgumentException(
						"Picked an unavailable option, choices were print file contents (0), make a request (1) or exit safely (2)");
			} else if (userresponse.equals("2")) {
				break;
			}
		}
	...
	public static void deleteEarly(int limit) {
		for (int i = 0; i < limit; i++) {
			File file = new File("src/resources/temp" + i + ".txt");
			file.delete();
		}
	}
